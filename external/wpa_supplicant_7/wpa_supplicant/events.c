...
// 2011-10-04, dongseok.ok@lge.com, Add EAP-FAST [START]
static int wpa_supplicant_eap_fast_init(struct wpa_supplicant *wpa_s,
                                        struct wpa_ssid *ssid)
{
	//if (wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt)) {
	if (NULL != ssid->eap.eap_methods) {
		int fast = 0;
		struct eap_method_type *eap = ssid->eap.eap_methods;
		while (eap->vendor != EAP_VENDOR_IETF ||
		       eap->method != EAP_TYPE_NONE) {
			if (eap->method == EAP_TYPE_FAST) {
				fast = 1;
			}
			eap++;
		}
		if(fast) {
			//if (access("/data/misc/wifi/fast-mschapv2.pac", F_OK)) {
			wpa_printf(MSG_INFO, "EAP-FAST: PAC file is exist, remove the file");
			system("rm /data/misc/wifi/fast-mschapv2.pac");
			//}
		}
	}
	//}
...
// 2011-10-04, dongseok.ok@lge.com, Add EAP-FAST [END]
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->current_ssid) {
        ...
        // 2011-10-04, dongseok.ok@lge.com, Add EAP-FAST [START]
		wpa_supplicant_eap_fast_init(wpa_s, wpa_s->current_ssid);
        // 2011-10-04, dongseok.ok@lge.com, Add EAP-FAST [START]
	}
