...
#ifndef ANDROID /*LGE_CHANGE, real-wifi@lge.com, [the icon networks available is not shown]*/
...
#ifndef ANDROID 	/* ANDROID will shutdown interface if we are disconnected. skip this */
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			reply_len = -1;
		else {
#endif            
			...
			wpa_supplicant_req_scan(wpa_s, 0, 0);
				...
				wpa_printf(MSG_DEBUG, "Ongoing scan action - "
					   "reject new request");
#ifndef ANDROID 	/* ANDROID do not reply with error code */
