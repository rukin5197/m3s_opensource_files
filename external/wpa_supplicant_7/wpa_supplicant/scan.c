...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wps) {
		...
		if (wps_ie) {
			...
			/* test case 5.1.17 : include WPS IE in broadcast probe req (scan) 
			 * In wext, there is no means to convey ie information in iw_scan_req 
			 * Instead, driver expects SIOCSIWGENIE before scan/connection */
			if(wpa_s->wps_setie_done == 0) {
				/* avoid subsquent set_gen_ie once wps is started */
				wpa_s->wps_setie_done = 1;
				wpa_drv_set_gen_ie(wpa_s, params.extra_ies, params.extra_ies_len);
			}
#endif
		}
	}
