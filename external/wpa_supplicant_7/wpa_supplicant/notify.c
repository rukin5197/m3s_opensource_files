...
#ifdef ANDROID
/* QCOM added  
 * to resolve mismatch wpa_state between 0.6.10 and 0.8.x
 * cause GUI behaves wrong
 *
 * input : wpa_state = supplicant wpa state 
 * output : and_state = corresponding android state
 * return 0 : do not notifiy. conversion won't happen. and_state is unmodified.
 * return 1 : do notify. conversion done 
 */
int wpa_state_map_to_android(enum wpa_states wpa_state, enum wpa_states *and_state)
{
	/* following state machines does not exisit in 0.6.10
	   do not notify to android 
	 */
	if(wpa_state == WPA_INTERFACE_DISABLED ||
		wpa_state == WPA_AUTHENTICATING)
		return 0;
	/* conversion needed */
	if(wpa_state < WPA_INACTIVE)
		*and_state = wpa_state;
	else if(wpa_state >= WPA_INACTIVE && wpa_state < WPA_ASSOCIATING)
		*and_state = wpa_state - 1;
	else /* if(wpa_state >= WPA_ASSOCIATING) */
		*and_state = wpa_state - 2;
