...
static void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	/* tell driver to clear WPS session */    
	wpa_drv_set_gen_ie(wpa_s, NULL, 0);
	wpa_s->wps_setie_done = 0;
#endif
}
			     ...
			     struct wpa_ssid *selected)
{
	...
	wpa_s->blacklist_cleared = 0;
#ifdef CONFIG_WPS2 /* QCOM added */
	/* before starting WPS_pbc/pin/reg connectio, reset this variable 
	   so that first scan will issue set_gen_ie() to driver */
	wpa_s->wps_setie_done = 0;
#endif
	wpa_supplicant_req_scan(wpa_s, 0, 0);
}
