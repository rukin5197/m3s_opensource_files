					      ...
					      u8 *buf, size_t len)
{
    ...
    /* QCOM comment out : this caused endless connection/disconnection with ANDROID GUI */
#ifndef ANDROID 
...
#endif
#ifdef CONFIG_WPS2 /* QCOM added */
    /* when WPS IE is present, send probe req (active scan) : test case 5.1.17 */
	int scan_need_active = (params->extra_ies && params->extra_ies_len);
#endif
...
#ifdef CONFIG_WPS2  /* QCOM added */
	if (scan_need_active || (scan_probe_flag && (ssid && ssid_len))) {
#else
	if (scan_probe_flag && (ssid && ssid_len)) {
#endif
#else
#ifdef CONFIG_WPS2  /* QCOM added */
	if (scan_need_active || (ssid && ssid_len)) {
#else
	if (ssid && ssid_len) {
#endif
#endif
...
#ifdef CONFIG_WPS2  /* QCOM added */
		req.scan_type = scan_need_active ? IW_SCAN_TYPE_ACTIVE : IW_SCAN_TYPE_PASSIVE;
#endif
...
int wpa_driver_wext_combo_scan(void *priv, struct wpa_driver_scan_params *params)
{
	...
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
	struct wpa_ssid *ssid_conf = (struct wpa_ssid *)(wpa_s->conf->ssid);
#ifdef CONFIG_WPS2 /* QCOM added */
	/* when WPS IE is present, send probe req (active scan) : test case 5.1.17 */
	int scan_need_active = (params->extra_ies && params->extra_ies_len);
#endif
	...
	wpa_printf(MSG_DEBUG, "%s: Start %p", __func__, params->ssids[0].ssid);

	bp = WEXT_CSCAN_HEADER_SIZE;
	os_memcpy(buf, WEXT_CSCAN_HEADER, bp);
	/* Set list of SSIDs; */
	if(params->ssids[0].ssid != NULL) {
		/* not wildcard scan; fill from params->ssids */
		while (i < WPAS_MAX_SCAN_SSIDS) {
			if ((bp + IW_ESSID_MAX_SIZE + 10) >= (int)sizeof(buf))
				break;
			if (params->ssids[i].ssid == NULL)
				break;
			wpa_printf(MSG_DEBUG, "For Specific Scan: %d", params->ssids[i].ssid_len);
		...
		while (i < WEXT_CSCAN_AMOUNT) {
			...
			if (!ssid->disabled && ssid->scan_ssid) {
				wpa_printf(MSG_DEBUG, "For Conf Scan: %d", ssid->ssid_len);
				buf[bp++] = WEXT_CSCAN_SSID_SECTION;
	...
	if(scan_need_active) {
		...
		buf[bp++] = (u8)WEXT_CSCAN_TYPE_DEFAULT;
	}
#endif
	os_memset(&iwr, 0, sizeof(iwr));
...
#endif
#ifdef CONFIG_WPS2 /* QCOM added */
	.set_gen_ie = wpa_driver_wext_set_gen_ie,
#endif
};
