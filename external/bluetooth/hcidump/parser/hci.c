...
static inline void create_physical_link_dump(int level, struct frame *frm)
{
	...
	printf("handle %d key length %d key type %d\n",
		cp->handle, cp->key_len, cp->key_type);

	p_indent(level, frm);
	printf("key ");

	for (i = 0; (i < cp->key_len) && (i < AMP_KEY_LEN_PAL); i++)
...
static inline void read_local_amp_info_dump(int level, struct frame *frm)
{
	...
	if (rp->status > 0)
		...
		printf("total bandwidth %d, max guaranteed bandwidth %d\n",
			   btohl(rp->total_bandwidth), btohl(rp->max_guarant_bandwidth));
		p_indent(level, frm);
		...
		printf("controller type 0x%2.2x, pal caps 0x%4.4x, max assoc length %d\n",
			   rp->controller_type, btohs(rp->pal_caps), btohs(rp->max_amp_assoc_len));
		p_indent(level, frm);
