...
static void bluetooth_close(struct bluetooth_data *data)
{
	DBG("bluetooth_close");
	if (data->server.fd >= 0) {
		// +s QCT_BT_PATCH_SR00670567 munho2.lee@lge.com 111205, fix issue that does not output when playing music via BT A2DP (a2dp_write fail).
		// liba2dp changes to have proper cleanup of unix sockets.
...
{
	// *s QCT_BT_PATCH_BT_WIFI_COEX suhui.kim@lge.com 111115, fix BT/Wi-Fi Co-ex issue (high quality -> middle quality)
	/* QCT109804 Original
	switch (freq) {
