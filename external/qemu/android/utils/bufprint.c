...
//20110822 inhwan.seo@lge.com FRAMEWORK_M3S_SPCS_INITIAL_MERGE [START]
    // LGE_CHANGE [dojip.kim@lge.com] 2010-07-03, fixed the running qemu in server
#if 1
    char path[MAX_PATH] = {0,};
    snprintf (path, MAX_PATH, "%s/%s", (const char *)getenv("HOME"), "/.qemu");
    if (access(path, F_OK) != 0)
        mkdir (path, 0755);
    strncat(path, "/android", MAX_PATH);
    mkdir(path, 0744);
    return bufprint(buff, end, "%s", path);
#else /* original code */
